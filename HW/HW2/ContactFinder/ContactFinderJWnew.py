...
# Iteration 4
epatterns.append(r'([A-Za-z.]+)\s+@\s+([A-Za-z.]+)\.[A-Za-z]+') # gets dabo
# Iteration 5
...
# Iteration 9
epatterns.append('([A-Za-z.]+)\s<at symbol>\s([A-Za-z.]+)\.edu')   # gets manning
# Iteration 10
epatterns.append('([A-Za-z.]+)<del>@([A-Za-z.]+)\.edu')   # gets latombe
# Iteration 11
...
"""
# JW Part 3 - Additional regular expressions
epatterns.append('([A-Za-z._0-9]+)\s*(?:@|\sat\s)\s*([A-Za-z._0-9]+)(?:\.|\s)([A-Za-z]+)')
epatterns.append('([A-Za-z._0-9]+)\s*(?:@|\sat\s)\s*([A-Za-z._0-9]+)(?:\.|\s)([A-Za-z]{3})')
epatterns.append('([A-Za-z._0-9]+)\s*(?:@|\sat\s)\s*([A-Za-z._0-9]+)(?:\.|\s)(com|edu)\W')
epatterns.append('([A-Za-z._0-9]+)\s*(?:@|\sat\s)\s*([A-Za-z.;_0-9]+)\s*(?:\.|\s|;)((?:com|edu))\W')
epatterns.append('([A-Za-z._0-9-]+)@([A-Za-z.;_0-9-]+)\.(-e-d-u)')

# JW altered process_file 
def process_file(name, f):
    ...
    for line in f:
        ...
        # matching DOT, dot, DOM and dt as the server name with the line
        ...
        for epat in epatterns:
            ...
            for m in matches:
                ...
                #email = '%s@%s.edu' % m
                # adding ways to split up the email into different parts
                if len(m) <= 2:
                    email = '{}@{}.edu'.format(m[0],m[1])
                # if the line comes in with 3 entities, we have to accomodate accordingly
                elif len(m) == 3:
                    eServe = re.sub( '(?:\s|\;)' , '.' , m[1])
                    eName = re.sub( '-' , '' , m[0])
                    eServe = re.sub( '-' , '' , eServe)
                    doMain = re.sub( '-' , '' , m[2])
                    email = '{}@{}.{}' .format(eName,eServe,doMain)
...
# comment out to run altered by Joyce Woznica version
#def process_file(name, f):
    # note that debug info should be printed to stderr
    # sys.stderr.write('[process_file]\tprocessing file: %s\n' % (path))
#    res = []
